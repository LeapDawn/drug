 ...
 * 阴性MIC导入/导出
 */
public class NegativeMicUpDownServieImpl extends MicUpDownService {
	...
	public UploadResultModel importDatas(InputStream input, String user)
		...
		List<PStrainMic> pageList = this.transferCodingForNegative(bodyList, errorList, user);
		List<StrainMic> addList = new ArrayList<StrainMic>();
		CheckCodingAttrs(addList, errorList, pageList, "阴性");
		...
		UploadResultModel result = new UploadResultModel(errorList,
				bodyList.size());
		return result;
	}
	...
	public File exportDatas(String nos, File file) throws ExcelException {
		...
		List<Map<String, Object>> bodyList = this.retransferForNegative(strainMics);
