...
public class PageResultModel<T> {
	...
	public PageResultModel(int total, int rows, int currentPage, List<T> data) {
		super();
	...
	public PageResultModel(int total, int rows) {
		this.total = total > 0 ? total : 0;
		this.rows = rows > 0 ? rows : 100;
	...
	public PageResultModel(int total, int rows, int currentPage) {
		this.total = total > 0 ? total : 0;
		this.rows = rows > 0 ? rows : 100;
		this.pages = this.total % this.rows == 0 && this.total != 0 ? (this.total / this.rows)
				: (this.total / this.rows) + 1;
	...
	public PageResultModel(int total, int rows, int currentPage, String order,
			String sort) {
		this.total = total > 0 ? total : 0;
		this.rows = rows > 0 ? rows : 100;
		this.pages = this.total % this.rows == 0 && this.total != 0 ? (this.total / this.rows)
